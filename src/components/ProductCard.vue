<template>
    <q-card class="product-card flex column">
        <q-img :src="getProductImage(product)" :alt="product.name" class="product-image" spinner-color="primary"
            spinner-size="82px">
...
function getProductImage(product) {
    // Если есть поле image и оно совпадает с imageMap — используем его (для локальных товаров)
    if (product.image && imageMap[product.image]) {
        return imageMap[product.image];
    }
    // Если есть категория — используем картинку по категории
...
const imageMap = {
    ...
    'potion.svg': potion,
    'book.svg': magicBook,
    // если в Firebase другие имена — добавьте их сюда
};
</script>
