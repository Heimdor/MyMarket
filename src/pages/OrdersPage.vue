        ...
        <q-table :rows="orders" :columns="columns" row-key="id" :pagination="{ rowsPerPage: 10 }" class="orders-table">
          <template v-slot:body="props">
            <q-tr :props="props" class="order-row">
              ...
              <q-td key="total" :props="props" class="text-weight-medium">{{ formatPrice(props.row.total) }} ₽</q-td>
                    ...
                    <div class="row justify-end q-gutter-y-xs">
                      <div class="col-12 col-md-5">
                        <div class="row items-center justify-end">
                          ...
                          <div class="text-body2 text-weight-medium">{{ formatPrice(selectedOrderSubtotal) }} ₽</div>
                        </div>
                        ...
                        <div class="row items-center justify-end">
                          ...
                          <div class="text-subtitle1 text-weight-bold text-primary">{{ formatPrice(selectedOrderTotal)
                            }} ₽</div>
                        </div>
                      </div>
                    </div>
...
const fetchOrders = async () => {
  ...
  try {
    ...
    // Преобразуем строковые значения в числа и пересчитываем суммы
...
// Добавьте эти вычисляемые свойства после определения selectedOrder
const selectedOrderSubtotal = computed(() => {
...
const selectedOrderTotal = computed(() => {
  ...
  const deliveryCost = parseFloat(selectedOrder.value.deliveryCost) || 0;
  return selectedOrderSubtotal.value + deliveryCost;
});
